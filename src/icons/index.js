...
(async () => {
  const fileList = await Promise.all(
    Object.values(svgModules).map(async (module) => {
      const { default: svgContent } = await module()
      return svgContent
    })
  )

  iconList = fileList.map(file => file.split('/svg')[1].replace('.svg', '').replace('/', ''))
})();
