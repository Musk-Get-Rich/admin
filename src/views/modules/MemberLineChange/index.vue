  ...
  <el-card>
    ...
    <div class="grid grid-cols-3 gap-10">
      <div class="flex flex-col justify-center p-8xl border border-solid border-gray-300 rounded-3xl">
        ...
        <div class="text-6xl text-gray-400 mt-5">{{ $t('本月转线') }}</div>
      </div>
      <div class="flex flex-col justify-center p-8xl border border-solid border-gray-300 rounded-3xl">
        <div class="font-bold text-8xl">{{ info.allNum || 0 }}</div>
        <div class="flex justify-between text-6xl text-gray-400 mt-5">
          <span>{{ $t('转线记录') }}</span>
          <el-button type="success" size="small" round plain @click="router.push('/managementCenter/memberLineChange/records')">{{ $t('查看详情') }}</el-button>
        </div>
      </div>
      <div class="flex flex-col justify-center p-8xl border border-solid border-gray-300 rounded-3xl">
        <div class="font-bold text-8xl">{{ info.monthRatio || "0.00%" }}</div>
        <div class="text-6xl text-gray-400 mt-5">{{ $t('转线率') }}</div>
      </div>
    </div>
...
<!--            </el-form-item>-->
      ...
      <div class="mt-7xl py-6xl">
        ...
        <div class="text-5xl color-grey">
          <div class="my-6">1、{{ $t('会员是在总线的，不在其他代理线下。') }} </div>
          <div class="my-6">2、{{$t('充值少于')}}6,000。 </div>
          <div class="my-6">3、{{ $t('提供登陆信息图片和其他证明在你线下的图片。输入需注销的会员账号系统将会识别是否符合') }}</div>
        </div>
      </div>
    </div>
          ...
          <el-carousel ref="carouselRef" indicator-position="outside" height="350px" trigger="click" arrow="never" :autoplay="false">
            <el-carousel-item>
              <div class="flex flex-col">
                ...
                <div class="mt-15">
                  <div>1.{{ $t('上传与会员的对话截图，至少包含注册链接、会员账号、日期。') }} </div>
                  <div>2.{{ $t('输入需转线的会员账号。') }}</div>
                </div>
              </div>
            </el-carousel-item>
            <el-carousel-item>
              <div class="flex flex-col">
                ...
                <div class="mt-15">
                  <div>3.{{ $t('输入发给会员的注册链接。') }}</div>
                  <div>4.{{ $t('选择会员设备类型（此项可以随意选择）完成以上步骤后即可提交申请') }}</div>
                </div>
              </div>
            </el-carousel-item>
          </el-carousel>
